...
# Start localtunnel for the frontend with a specific subdomain
echo -e "${GREEN}Starting localtunnel to create HTTPS tunnel to frontend...${NC}"
lt --port 3004 --subdomain nedoma-dev --local-host localhost > tunnel.log &
TUNNEL_PID=$!

# Wait for tunnel to start and get URL
sleep 5
TUNNEL_URL=$(grep -o 'https://[^[:space:]]*' tunnel.log | head -n 1)
