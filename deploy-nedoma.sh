...
SERVER_USER="root"
SERVER_PASSWORD="0QEzDjwngNxXNJ9O"
GITHUB_REPO="https://github.com/gri4014/nedoma.git"
APP_DIR="/opt/nedoma"
DOMAIN="nedoma.site"

# Create the deployment script
cat > deploy-server.sh << 'EOL'
#!/bin/bash
...
sshpass -p "${SERVER_PASSWORD}" scp deploy-server.sh ${SERVER_USER}@${SERVER_IP}:/root/
sshpass -p "${SERVER_PASSWORD}" ssh ${SERVER_USER}@${SERVER_IP} "cd /root && chmod +x deploy-server.sh && ./deploy-server.sh"
